...
function generateCodeFromObject(obj){
    ...
    if(obj.style!==undefined && Object.keys(obj.style).length>0){
        ...
        for(let i=0;i<Object.keys(obj.style).length;i++){
            ans_str+=camelize(style_keys[i])+':"'+obj.style[style_keys[i]]+'",';
        }
...
//         /\w\s*/g,
//         function(text){
//             return text.charAt(0).toUpperCase()+text.substr(1,text.length).toLowerCase();
//         }
//     ).replace(" ","");
// }
...
// const camleCase=(str)=>{
//     str= str.replace("-"," ")
//               .replace(/\s(.)/g,function(text){return text.toUpperCase();})
//               .replace(/\s/g,"")
//               .replace(/^(.)/g,function(text){return text.toLowerCase();});
//     return str;
// }
